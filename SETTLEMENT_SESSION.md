...
> 하나의 서사: **순진하게 스레드를 늘려 터뜨리고(4-1) → 멀티스레드가 데이터를 꼬는 걸 목격하고(4-2)
> → 데이터를 안 겹치게 나누는 게 정답임을 깨닫는다(4-3, 파티셔닝).**

| 구성 | 파일 | 설명 |
|---|---|---|
| 비동기화 공유 Reader | `batch/UnsafeSharedOrderReader` | **일부러 thread-safe 하지 않게 만든** 페이징 Reader. 공유 `cursor` 경쟁으로 같은 주문이 두 스레드에 들어간다(4-2 함정) |
| 범위 분할 마스터 | `batch/OrderRangePartitioner` | PAID 주문의 `id` 범위를 `gridSize` 개로 균등 분할 → 각 파티션에 `minId/maxId` 전달 (4-3) |
| 워커 구성 | `batch/SettlementParallelJobConfig` | `@StepScope` 워커 Reader(자기 범위만 읽음) + 워커 Step. 파티션마다 Reader **독립** |
| Job 조립기 | `batch/SettlementParallelJobFactory` | 스레드 수/gridSize 를 런타임에 받아 멀티스레드 Job·파티셔닝 Job 을 동적으로 생성 |
| 실행기 | `application/SettlementBatchService` | `runMultiThreaded(threads, safe)`, `runPartitioned(gridSize)` |
| 엔드포인트 | `POST /settlements/batch/multi-threaded?threads=N&safe=true|false`, `POST /settlements/batch/partitioned?gridSize=N` | 4-1/4-2 / 4-3 트리거 |
| 풀·병렬도 설정 | `application.properties` | `spring.datasource.hikari.*`, `settlement.batch.thread-count/grid-size` |

#### ⚠️ 강사 필독 — Spring Batch 6 에서 바뀐 것 (이 코드가 왜 이렇게 생겼나)

선행학습 자료의 "Multi-threaded Step → Paging Reader 가 thread-safe 하지 않아 중복/누락" 은
**Spring Batch 4~5 기준**이다. 이 프로젝트(Spring Boot 4.1 / **Batch 6.0.4**)에서는 두 가지가 바뀌었다.

1. **표준 `JpaPagingItemReader.read()` 가 내부 `Lock` 으로 보호된다.** → 표준 Reader 만으로는 인덱스
   꼬임이 재현되지 않는다(선행학습의 "`SynchronizedItemStreamReader` 반창고" 가 이미 내장된 셈).
2. **신형 `chunk(size)`(`ChunkOrientedStep`) 에 `taskExecutor` 를 주면 "읽기는 단일 스레드, 가공만 병렬"** 이다.
   (= 선행학습의 'Async 카드'!) 즉 신형 API 로는 "여러 스레드가 Reader 하나를 공유" 하는 전통 함정이
   **구조적으로 안 생긴다.**

그래서 함정(4-2)을 **진짜로** 보여주려고 이 코드는 ① 동기화를 뺀 `UnsafeSharedOrderReader` 를,
② 여러 스레드가 청크를 통째로 도는 **레거시 빌더** `chunk(size, txManager)` + `taskExecutor(TaskExecutor)`
(`TaskExecutorRepeatTemplate`)를 **의도적으로** 쓴다. 이 레거시 빌더는 Batch 6 에서 `@Deprecated(forRemoval)` 다 —
"바로 이런 위험 때문에 비권장됐다" 는 게 또 하나의 토크 포인트.

#### [Step4-1] 스레드만 늘리면 터진다 — 커넥션 풀 (`safe=true` 로 데모)

`safe=true` 는 **표준(thread-safe) Reader** 를 쓰므로 데이터는 안 꼬이고 **오직 커넥션 풀 고갈만** 드러난다.
```bash
# 풀을 일부러 작게(2) 걸고 기동
./gradlew bootRun --args='--settlement.seed.enabled=true --settlement.seed.count=5000 \
  --settlement.batch.chunk-size=100 --spring.datasource.hikari.maximum-pool-size=2 \
  --spring.jpa.show-sql=false --logging.level.org.hibernate.orm.jdbc.bind=off'
```
```http
POST /settlements/batch/multi-threaded?threads=10&safe=true
```
- 결과: `status=FAILED`, 로그에 **`HikariPool-1 - Connection is not available, request timed out after 3000ms`**.
- 처방: `--spring.datasource.hikari.maximum-pool-size=20` 으로 키워 재기동 → **같은 호출이 COMPLETED(5000)** 로 산다.
  - 동시에 "표준 Reader 는 멀티스레드에서도 데이터가 안 꼬인다(=Batch 6 가 read() 를 락으로 보호)" 도 같이 증명된다.
- 토크 포인트: 병렬화는 스레드 수만의 문제가 아니라 **공유 자원(커넥션)** 과의 균형. 무한정 키우면 이번엔 DB 가 죽는다.

#### [Step4-2] 진짜 함정 — 멀티스레드가 데이터를 꼬는다 (`safe` 생략 = 비동기화 Reader)

풀을 충분히(20) 키운 상태에서, 이번엔 **비동기화 공유 Reader** 로 돌린다.
```http
POST /settlements/batch/multi-threaded?threads=8
```
- 결과(이 코드베이스): 공유 `cursor` 경쟁으로 **같은 주문이 두 스레드에** 들어가고, Step3 에서 건
  `order_id` **UNIQUE 제약**이 '이중정산'을 막아 **`status=FAILED`**. 그것도 **매번 다른 지점, 커밋 건수도 들쭉날쭉**(예: 100, 200, …)한 **비결정적** 실패다.
- 핵심 해석: **Step3 의 UNIQUE/멱등이 '이중정산 행'은 막아 주지만, 멀티스레드가 만든 '누락'과 '깨짐'은 못 막는다.**
  만약 UNIQUE 가 없었다면 → 조용한 이중정산·정산 누락 **사고**. 에러보다 무서운 데이터 오염.
- 토크 포인트: 잘못은 "일꾼을 여러 명 둔 것"이 아니라 **"여러 일꾼이 같은 데이터 줄을 공유하며 경쟁한 것"**.
  `SynchronizedItemStreamReader`(read() 동기화)는 중복은 막지만 읽기를 직렬화하는 **반창고**다. → 구조적 해법으로.

#### [Step4-3] 정답 — 데이터를 나눠준다 (Partitioning)

마스터(`OrderRangePartitioner`)가 `id` 범위를 `gridSize` 등분해 **워커마다 겹치지 않는 구역**을 주고,
워커는 **자기 전용 `@StepScope` Reader** 로 그 범위만 읽는다. 공유가 없으니 4-2 의 경쟁이 **원천적으로** 사라진다.
```http
POST /settlements/batch/partitioned?gridSize=8
```
- 결과: `status=COMPLETED`, **정확히 5000건, 중복 0** (`SELECT COUNT(*), COUNT(DISTINCT order_id) FROM settlements` 일치).
- 멱등·재시작도 그대로: 각 워커가 `PENDING`만 읽고(멱등 스킵), UNIQUE 가 최후 방어, 파티션 단위 재시작.
  → **빨라지면서도 "1원의 무결성"은 그대로.**
- `gridSize` 주의: 워커 수 > 커넥션 풀이면 4-1 처럼 풀이 터진다(파티셔닝도 예외 아님). 또 어느 지점부터는
  DB·IO·코어가 진짜 병목이라 더 안 빨라진다.

|  | Multi-threaded Step (4-2) | Partitioning (4-3) |
|---|---|---|
| Reader | **하나를 공유**(경쟁) | 파티션마다 **따로**(독립) |
| thread-safety | 깨지기 쉬움(중복→UNIQUE 실패/누락) | 구조적으로 안전 |
| 재시작 | 까다로움 | 파티션 단위로 깔끔 |
| 한 줄 비유 | 한 줄에서 서로 밀치기 | 구역을 나눠 각자 일하기 |

#### 별도 카드: Async (개념만, 구현 생략)
`AsyncItemProcessor`/`AsyncItemWriter` 는 **읽기는 빠른데 가공(Processor)이 무거울 때** 꺼내는 카드다.
재미있는 사실: **Batch 6 의 신형 `chunk(size).taskExecutor(...)` 가 사실상 이 동작**(읽기 단일·가공 병렬)이다.
파티셔닝(=데이터 분할)과는 결이 다르다. 오늘은 "이런 게 있다"만 알고 넘어간다.
